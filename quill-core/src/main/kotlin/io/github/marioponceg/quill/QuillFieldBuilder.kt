 ...
 * strings that parse as JSON and arbitrary objects (via `toString()`) to
...
public class QuillFieldBuilder internal constructor() {
    ...
    private fun convert(value: Any?): QuillValue = when (value) {
        ...
        is String ->
            if (JsonPrettyPrinter.prettyPrintOrNull(value) != null) {
                QuillValue.Structured(value)
            } else {
                QuillValue.Text(value)
            }
